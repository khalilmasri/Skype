...
// SEARCH <user> (searches the whole user list in the server for a given
// username ).
// ADD   <user>
// REMOVE <user>
// PING  (it might be a good idea for the
// client to ping every second or so to confirm a client is still online )
// AVAILABLE <user> (we can use this for a quick check on a given user/conect
// status and IP before connecting )
